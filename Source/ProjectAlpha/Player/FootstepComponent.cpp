...
void UFootstepComponent::BeginPlay()
{
	...
	Owner = CastChecked<APA_Character>(GetOwner());
	OwnerController = CastChecked<APA_Controller>(Owner->GetController());
...
void UFootstepComponent::PlayFootstepCameraShake() const
{
	if (FootstepDataAsset)
	{
		...
		{
			if (const USettingsSubsystem* Subsystem = GetWorld()->GetGameInstance()->GetSubsystem<USettingsSubsystem>())
			{
				if (Subsystem->bIsScreenShake)
				{
